...
SELECT 
	...
	, T.NAme AS 'TableName'
	, 'EXEC xp_cmdshell ''bcp "SELECT * FROM  AdventureWorks' + s.Name +'.'+  T.NAme + '" queryout "G:\Adatis\SQLBits - SQLDW Planning - General\ExportedData\MagicWorksDW\' + s.Name +'_'+  T.NAme + '.txt" -T -c -t''' AS 'Export'
...
SELECT 
	...
	, 'AzCopy /Source:"G:\Adatis\SQLBits - SQLDW Planning - General\ExportedData\MagicWorks" /Dest:https://magicworksblob.blob.core.windows.net/bcptest /DestKey:WR6PLCnUMJ9Hu6Wkt7EUadRLDnoVF3cTabiGm//3FBXXJOSFAPqjrkfqqEW9qT4P2OlsKDcY0iSRUfDWtNhKrA== /Pattern:"bcptest.txt"'
FROM
	sys.tables t 
	INNER JOIN sys.schemas S ON S.schema_id = t.schema_id
